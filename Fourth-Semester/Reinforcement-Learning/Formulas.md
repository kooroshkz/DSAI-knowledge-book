...
* **Bellman Optimality Equation for $v^*(s)$:** $v^*(s) = \max_a \sum_{s'} p(s'|s,a) [ r(s,a,s') + \gamma v^*(s') ]$
...
* **Optimal policy (deterministic):** $\pi^*(a|s) = \arg\max_a q^*(s,a)$
- **Bellman Optimality Equation:** $v^*(s): v^*(s) = \max_a \sum_{s'} p(s'|s,a) \left[ r(s,a,s') + \gamma v^*(s') \right]$
- **P non-greedy action = e/|A| , P greedy action = 1 - e + e/|A|**
* **Softmax (Boltzmann) Policy:** $P(a) = e^{Q(a)/\tau} / \sum_b e^{Q(b)/\tau}$
